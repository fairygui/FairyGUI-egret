namespace fgui {
    export class GPath {
        ...
        private _points: Array<egret.Point>;
        private _fullLength: number;

        private static helperPoints: Array<egret.Point> = new Array<egret.Point>();

        constructor() {
            this._segments = new Array<Segment>();
            this._points = new Array<egret.Point>();
        }
        ...
        public create(points: Array<GPathPoint>): void {
            ...
            var splinePoints: Array<egret.Point> = GPath.helperPoints;
        ...
        public getPointAt(t: number, result?: egret.Point): egret.Point {
            if (!result)
        ...
        public getAnchorsInSegment(segmentIndex: number, points?: Array<egret.Point>): Array<egret.Point> {
            if (points == null)
                points = new Array<egret.Point>();
        ...
        public getPointsInSegment(segmentIndex: number, t0: number, t1: number, points?: Array<egret.Point>, ts?: Array<number>, pointDensity?: number): Array<egret.Point> {
            if (points == null)
                points = new Array<egret.Point>();
            ...
            var seg: Segment = this._segments[segmentIndex];
            if (seg.type == CurveType.Straight) {
                points.push(new egret.Point(ToolSet.lerp(this._points[seg.ptStart].x, this._points[seg.ptStart + 1].x, t0),
                    ToolSet.lerp(this._points[seg.ptStart].y, this._points[seg.ptStart + 1].y, t0)));
                points.push(new egret.Point(ToolSet.lerp(this._points[seg.ptStart].x, this._points[seg.ptStart + 1].x, t1),
        ...
        public getAllPoints(points?: Array<egret.Point>, ts?: Array<number>, pointDensity?: number): Array<egret.Point> {
            if (points == null)
                points = new Array<egret.Point>();
            if (!pointDensity || isNaN(pointDensity))
                pointDensity = 0.1;
        ...
        private onCRSplineCurve(ptStart: number, ptCount: number, t: number, result: egret.Point): egret.Point {
        ...
        private onBezierCurve(ptStart: number, ptCount: number, t: number, result: egret.Point): egret.Point {
            var t2: number = 1 - t;
