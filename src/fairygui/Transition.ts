        ...
        public setup(xml: any) {
            ...
            for(var i1: number = 0;i1 < length1;i1++) {
                ...
                if(cxml.name == "item") {
                    ...
                    item.time = parseInt(cxml.$time) / this.FRAME_RATE;
                    ...
                    if(item.tween) {
                        item.duration = parseInt(cxml.$duration) / this.FRAME_RATE;
                        str = cxml.$ease;
                        if(str)
                        ...
                        if(v) {
                            this.decodeValue(item.type,cxml.$startValue,item.startValue);
                            this.decodeValue(item.type,v,item.endValue);
                        }
                        else {
                            item.tween = false;
                            this.decodeValue(item.type,cxml.$startValue,item.value);
                        }
                    }
                    else {
                        this.decodeValue(item.type,cxml.$value,item.value);
                    }
                }
            }
        }
